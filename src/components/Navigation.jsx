...
const Navigation = () => {
  ...
  const { isLoggedIn, isAdmin, profile, loginWithGoogle, logout } = useAuth()
  ...
  // Expose so GameDetailModal can trigger login
  Navigation.openAuth = () => setShowAuth(true)
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              {isLoggedIn ? (
                <div className="flex items-center gap-3">
                  {isAdmin && <span className="text-xs bg-red/10 text-red px-2 py-1 rounded-full font-bold">{t('nav.admin')}</span>}
                  <div className="flex items-center gap-2">
                    {avatarUrl ? <img src={avatarUrl} alt="" className="w-8 h-8 rounded-full" /> : <div className="w-8 h-8 rounded-full bg-navy/10 flex items-center justify-center"><User className="w-4 h-4 text-navy" /></div>}
                    <span className="text-sm font-bold text-gray-700">{displayName}</span>
                  </div>
                  <button onClick={logout} className="flex items-center gap-1.5 text-sm font-semibold text-gray-400 hover:text-red transition-colors"><LogOut className="w-4 h-4" /></button>
                </div>
              ) : (
                <button onClick={() => setShowAuth(true)} className="flex items-center gap-2 bg-navy text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-navy-dark transition-colors">{t('nav.login')}</button>
              )}
            </div>
              ...
              <div className="px-4 py-4 flex flex-col gap-3">
                ...
                {isLoggedIn ? (
                  <div className="flex items-center justify-between py-2">
                    <div className="flex items-center gap-2">
                      {avatarUrl ? <img src={avatarUrl} alt="" className="w-7 h-7 rounded-full" /> : <div className="w-7 h-7 rounded-full bg-navy/10 flex items-center justify-center"><User className="w-3.5 h-3.5 text-navy" /></div>}
                      <span className="text-sm font-bold text-gray-700">{displayName}</span>
                      {isAdmin && <span className="text-[10px] bg-red/10 text-red px-1.5 py-0.5 rounded-full font-bold">ADMIN</span>}
                    </div>
                    <button onClick={() => { logout(); setMobileOpen(false) }} className="text-sm font-semibold text-red flex items-center gap-1"><LogOut className="w-4 h-4" /></button>
                  </div>
                ) : (
                  <button onClick={() => { setShowAuth(true); setMobileOpen(false) }} className="w-full bg-navy text-white py-3 rounded-lg text-sm font-bold">{t('nav.login')}</button>
                )}
              </div>
            </motion.div>
          )}
        ...
        {showAuth && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4" onClick={closeAuth}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-8">
                <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-200 text-gray-700 py-3.5 rounded-xl font-bold hover:bg-gray-50 hover:border-gray-300 transition-all text-sm">
                  <GoogleIcon />
                  {t('auth.signInGoogle')}
                </button>
                {authError && <p className="text-red text-sm bg-red-50 px-3 py-2 rounded-lg mt-4">{authError}</p>}
