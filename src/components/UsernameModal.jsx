...
const UsernameModal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validation.valid) { setError(t(`username.${validation.error}`)); return }
    setLoading(true)
    const result = await createProfile(username)
    if (!result.success) setError(result.message === 'Username already taken' ? t('username.taken') : result.message)
    setLoading(false)
  }
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">{t('username.label')}</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 text-sm font-bold">@</span>
                  <input type="text" placeholder="your_name" value={username}
                  ...
                  {username.length >= 3 && (
                    <span className="absolute right-3 top-1/2 -translate-y-1/2">
                      {isValid ? <Check className="w-4 h-4 text-emerald-500" /> : <AlertCircle className="w-4 h-4 text-red" />}
                    </span>
                  )}
                </div>
                <p className="text-[11px] text-gray-400 mt-1.5">{t('username.rules')}</p>
              </div>
              {error && (
