...
const AdminJSONPanel = ({ onGamesUpdated, currentDate }) => {
  ...
  const handlePreview = () => {
    ...
    try {
      ...
      if (!parsed.game_date || !parsed.games || !Array.isArray(parsed.games)) {
        throw new Error('JSON must have "game_date" (string) and "games" (array).')
      }

      const rows = parsed.games.map(g => ({
        game_date: parsed.game_date,
        game_time: g.game_time || 'TBD',
        status: g.status || 'upcoming',
        is_value_pick: g.is_value_pick || false,
        is_featured: g.is_featured || false,
        home_team_name: g.home?.name || '',
        home_team_abbr: g.home?.abbr || '',
        home_team_record: g.home?.record || '',
        home_team_color: g.home?.color || '#1D428A',
        home_team_mascot_name: g.home?.mascot_name || '',
        away_team_name: g.away?.name || '',
        away_team_abbr: g.away?.abbr || '',
        away_team_record: g.away?.record || '',
        away_team_color: g.away?.color || '#333333',
        away_team_mascot_name: g.away?.mascot_name || '',
        home_win_pct: g.prediction?.home_win_pct || 50,
        away_win_pct: g.prediction?.away_win_pct || 50,
        data_points: g.prediction?.data_points || 0,
        home_streak: g.home?.streak || '',
  ...
  const handlePublish = async () => {
    ...
    if (result.success) {
      setSuccess(`${preview.length} games published!`)
      setPreview(null)
      setJsonInput('')
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm(`Delete ALL games for ${deleteDate}?`)) return
    ...
    if (result.success) {
      setSuccess(`All games for ${deleteDate} deleted.`)
      if (onGamesUpdated) onGamesUpdated()
    } else {
      setError(result.error)
    }
          ...
          >
            ...
            >
              ...
              <div className="bg-navy p-6 flex items-center justify-between sticky top-0 z-10">
                <div>
                  <h2 className="text-white text-xl font-bold">Game Manager</h2>
                  <p className="text-blue-200 text-sm">Upload and manage NBA games</p>
                </div>
              ...
              <div className="p-6 flex flex-col gap-6">
                ...
                {success && (
                  <div className="flex items-start gap-2 bg-green-50 border border-green-200 text-green-700 p-3 rounded-lg text-sm">
                ...
                <div>
                  ...
                  <button
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-2 bg-navy text-white py-3 rounded-xl font-bold hover:bg-navy-dark transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                ...
                {preview && (
                  <div>
                    <h3 className="text-sm font-bold text-gray-700 mb-3">
                      Preview — {preview.length} game{preview.length !== 1 ? 's' : ''} for {preview[0]?.game_date}
                    </h3>
